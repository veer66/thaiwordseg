...
dump_iterator_info (WcDictIter * self)
{
  ...
  switch (self->status)
    {
    case WC_DICT_ITER_ACTIVE:
      printf ("ACTIVE");
      break;
    case WC_DICT_ITER_DEAD:
      printf ("DEAD");
      break;
    default:
      printf ("ERR");
...
read_tab (WcDict * self, FILE * fp)
{
  ...
  WC_RET_CHECK (fread (self->tab, 1, self->size, fp) == self->size);
  return WC_RET_NORMAL;
}
...
dump_node (WcDict * self, wc_uint32 p, int level)
{
  ...
  switch (type)
      ...
      for (i = 0; i < child_count; i++)
	{
	  printf ("Child = 0x%X\n",
		  child =
		  (*(wc_uint32 *) & self->tab[p + i * 3] & 0x00FFFFFF));
      ...
      for (i = 0; i < len; i++)
	{
      ...
      printf ("-----------------------------------\n");
      printf ("Child = 0x%X\n", child =
	      (*(wc_uint32 *) & self->tab[p] & 0x00FFFFFF));
      dump_node (self, child, level + 1);
      printf ("+++++++++++++++++++++++++++++++++++\n");
      break;
    default:
	  ...
	  for (i = 0; i < len; i++)
	    {
	      ...
	      child =
		*(wc_uint32 *) & self->tab[p + len + i * 3] & 0x00FFFFFF;
